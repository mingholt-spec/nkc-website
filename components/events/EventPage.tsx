...
export default function EventPage({ campaign }: Props) {
  const { pageConfig, contentBlocks, formConfig, eventDetails, mode, htmlContent } = campaign;
  ...
  const hasHtmlBlocks = contentBlocks?.some(b => b.type === 'html') ?? false;
  ...
  // Sticky CTA: watch when form scrolls out of view (stacked layout only)
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Bridge to the registration system during migration
    window.open(`https://bjj-manager-pro.web.app/event/${campaign.slug}`, '_self');
  };
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center gap-2 mb-2">
          <svg className="w-4 h-4 shrink-0" style={accent ? { color: accent } : { color: '#e50401' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
    ...
    return (
      <div className="space-y-2">
        ...
        <div className="w-full h-1.5 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${isUrgent ? 'bg-red-500' : ''}`}
    ...
    <form onSubmit={handleSubmit}>
            ...
            {field.type === 'guardianInfo' ? (
              <div className="p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800 space-y-2">
                ...
                <div className={`grid ${compact ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-3'} gap-2`}>
                  <input name="guardianName" placeholder="Namn" required={field.required} className={compact ? sidebarInputClasses : inputClasses} style={accent ? { '--tw-ring-color': accent } as React.CSSProperties : undefined} />
                  <input name="guardianEmail" type="email" placeholder="E-post" required={field.required} className={compact ? sidebarInputClasses : inputClasses} style={accent ? { '--tw-ring-color': accent } as React.CSSProperties : undefined} />
                  <input name="guardianPhone" type="tel" placeholder="Telefon" required={field.required} className={compact ? sidebarInputClasses : inputClasses} style={accent ? { '--tw-ring-color': accent } as React.CSSProperties : undefined} />
                </div>
              </div>
            ) : (
              <input
      ...
      <label className={`flex items-start gap-3 ${compact ? 'mt-3' : 'mt-4'} cursor-pointer`}>
        <input
          ...
          style={accent ? { accentColor: accent } : { accentColor: '#e50401' }}
        />
    ...
    <div className={contentBg}>
      {hasContentBlocks ? (
        <div className="campaign-blocks">
          {contentBlocks!.map(block => <BlockRenderer key={block.id} block={block} />)}
        </div>
      ) : pageConfig.description ? (
        <div className="px-6 sm:px-10 pt-8 sm:pt-10 pb-6">
          <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed text-base sm:text-lg font-medium">
            {pageConfig.description}
          </p>
        </div>
      ) : null}
    ...
    <div className={`${pageBg} min-h-screen`}>
      ...
      <header className="relative h-[40vh] sm:h-[50vh] md:h-[65vh] min-h-[350px] md:min-h-[500px]">
        ...
        <div className="absolute bottom-10 md:bottom-20 left-0 right-0 px-6 sm:px-10 max-w-6xl mx-auto">
          <h1
            className={`${TITLE_SIZE_CLASSES[pageConfig.titleSize ?? 'xl']} font-black tracking-tighter uppercase leading-[0.85] text-white mb-4`}
          >
            {pageConfig.title}
          </h1>
          {pageConfig.instructor && (
            <p
              className="text-lg sm:text-2xl md:text-3xl font-black uppercase tracking-[0.2em] opacity-90"
              style={{ color: accent ?? '#e50401' }}
            >
      ...
      {/* ── QUICK INFO BAR (stacked layout only) ── */}
        ...
        <main className={`relative z-10 ${hasHtmlBlocks ? 'w-full max-w-[100vw] overflow-x-hidden' : 'max-w-6xl mx-auto px-4 sm:px-8 py-6 md:py-10'}`}>
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className={`hidden lg:block w-[360px] flex-shrink-0 ${hasHtmlBlocks ? 'pr-8 pt-6' : ''}`} style={{ position: 'relative', zIndex: 1 }}>
              <div className="sticky top-6" ref={sidebarFormRef}>
                <FormCard compact />
              </div>
            </div>
          </div>
          ...
          {hasHtmlBlocks ? (
            <>
              <div className="campaign-blocks" style={{ position: 'relative', zIndex: 0 }}>
                {Array.isArray(contentBlocks) && contentBlocks.map(block => <BlockRenderer key={block.id} block={block} />)}
              </div>
      ...
      {/* ── STICKY CTA BAR (stacked + not sold out) ── */}
      {!isSidebar && showStickyCta && !isSoldOut && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-lg border-t border-zinc-200 dark:border-zinc-800 shadow-2xl animate-in slide-in-from-bottom duration-300">
          <div className="max-w-4xl mx-auto px-4 sm:px-8 py-3 flex items-center justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              {isPaid && (
                <span className="text-lg font-black text-zinc-900 dark:text-white tracking-tight shrink-0">
                  {price} kr
                </span>
              )}
              {maxAttendees > 0 && (
...
// ── Share row (client-side only for URL) ──
function ShareRow({ title, accent }: { title: string; accent: string }) {
  ...
  const share = (getShareUrl: (u: string, t: string) => string) =>
    window.open(getShareUrl(url, title), '_blank', 'noopener,noreferrer');
  ...
  const SHARE_TARGETS = [
    ...
    {
      label: 'X',
      getUrl: (u: string, t: string) => `https://twitter.com/intent/tweet?url=${encodeURIComponent(u)}&text=${encodeURIComponent(t)}`,
      viewBox: '0 0 24 24',
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {SHARE_TARGETS.map(btn => (
        ...
        >
          <svg className="w-3.5 h-3.5" fill="currentColor" viewBox={btn.viewBox}><path d={btn.path} /></svg>
        </button>
      ))}
      <button
        onClick={handleCopy}
        ...
        style={{ backgroundColor: copied ? `${accent}20` : 'rgba(161,161,170,0.2)', color: copied ? accent : undefined }}
      >
        {copied ? (
