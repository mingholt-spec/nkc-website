...
export default async function BlogBlock({ block }: Props) {
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-12">
      ...
      {posts.length === 0 ? (
        <p className="text-sm text-zinc-400 text-center py-8">Inga inlägg publicerade</p>
      ) : (
          ...
          {posts.map(post => {
            ...
            return (
              <a key={post.id} href={href}
                className="group block bg-white rounded-2xl border border-zinc-200 overflow-hidden hover:shadow-lg hover:border-zinc-300 transition-all duration-300">
                {post.coverImage && (
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-sm font-black uppercase tracking-tight text-zinc-900 mb-2 group-hover:opacity-70 transition-opacity line-clamp-2">
                    {post.title}
                  </h3>
