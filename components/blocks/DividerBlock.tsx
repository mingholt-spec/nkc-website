import type { PageBlockDivider } from '@/lib/types';
import { spacingToStyle, hasSpacing } from './blockSpacing';
import { blockStyleToCSS } from './blockStyle';
interface Props { block: PageBlockDivider }
export default function DividerBlock({ block }: Props) {
  const hasCustomPadding = hasSpacing(block.spacing);
  const sectionStyle = { ...spacingToStyle(block.spacing, undefined, { x: '24px', y: '8px' }), ...blockStyleToCSS(block.style) };
  return (
    <div className={`mx-auto max-w-4xl ${hasCustomPadding ? '' : 'px-6 py-2'}`} style={sectionStyle}>
      <hr className="border-zinc-200" />
    </div>
  );
}
